...
import { DetailPengiriman as DetailPengirimanType, Pengiriman } from "@/types";
...
const DetailPengiriman = () => {
  ...
  const [trackPengiriman, setTrackPengiriman] = useState<
    DetailPengirimanType[]
  >([]);
  const [image, setImage] = useState<string>();

  useEffect(() => {
    const getPengirimanAndDetail = async () => {
      if (resi) {
        const resultPengiriman = await ServicePengiriman.findDataPengiriman(
          resi
        );
        setPengiriman(resultPengiriman.data);
        const resultDetailPengiriman =
          await ServiceDetailPengiriman.getDataDetailPengiriman(resi);
        setTrackPengiriman(resultDetailPengiriman.data);
      }
    };

    getPengirimanAndDetail();
  }, [resi]);
          ...
          <Table className="mt-5">
            ...
            <TableBody>
                {trackPengiriman.map((item, index) => (
                    <TableRow key={index}>
                        <TableCell className="pl-0" width="25%">{item.gudang.nama}</TableCell>
                        <TableCell width="25%">{item.tanggal_sampai.toString()}</TableCell>
