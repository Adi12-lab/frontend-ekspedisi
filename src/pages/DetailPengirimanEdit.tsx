...
const DetailPengirimanEdit = () => {
  ...
  useEffect(() => {
    const getDetailPengiriman = async () => {
      if(resi) {
        const result = await ServiceDetailPengiriman.getDataDetailPengiriman(resi);
  ...
  return ( 
    <section className="mt-5">
        ...
        <div>
          <h1 className="font-bold text-2xl">Track Pengiriman {resi as string}</h1>
          <AddDetailPengiriman setRefresh={setRefresh} setNotify={setNotify} resi={resi as string}/>
        </div>
                ...
                <TableCell>{item.tanggal_sampai.toString()}</TableCell>
                  <TableCell className="flex space-x-3">
                    <Button variant="warning"
                       onClick={() => {
                        setSelected({ detailPengiriman: item, operation: "edit" })
                        setOpen(true)
                       }
                      }
                    >
                    ...
                    <Button
                      variant="destructive"
                      onClick={() => {
                        setSelected({ detailPengiriman: item, operation: "delete" });
                        setOpen(true);
                      }}
                    >
...
export default DetailPengirimanEdit;